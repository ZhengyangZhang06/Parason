...
def _apply_groupwise_parallel_bonus(
    ...
    latency_signal = np.array(
        [_safe_float((extra_infos[i] or {}).get("total_num_tokens", 0.0)) for i in range(n)], dtype=np.float32
    )
...
class RewardManagerWithServer:
    ...
    def __call__(self, data: DataProto, return_dict: bool = False, correctness_as_reward: bool = False):
        ...
        if "rm_scores" in data.batch.keys():
            ...
            # Recompute groupwise parallel bonus on the full batch.
            # The agent loop computed it per-item (group_size=1 → z=0);
            # here we have the complete batch so the z-scores are meaningful.
            if has_bonus_fields and uid_values is not None:
                ...
                # Update rm_scores at the last-response-token positions
                ...
                for info in extra_infos:
                    for k, v in info.items():
                        extra_infos_dict.setdefault(k, []).append(v)
            ...
            # Original short-circuit (no bonus recomputation needed)
            if return_dict:
